...
| `create_reply_draft` | Создать черновик «Ответить всем» в исходной переписке с дополнительными адресатами, упоминаниями и вложениями; без отправки | |
| `update_draft` | Изменить черновик; непереданные поля не трогаются, `attachments` (если передан) полностью заменяет набор вложений | |
| `send_draft` | Отправить черновик и проверить его копию в «Отправленных» | |
...
- Прямая отправка возвращает `submitted`, что не означает доставку получателю.
  `send_draft` проверяет отправленную копию и возвращает её новый ID; старый ID
  черновика не используется как ID отправленного письма.
...
## Ответы в черновиках, упоминания и подтверждение отправки

`create_reply_draft` создаёт настоящий EWS-ответ для проверки перед отправкой.
По умолчанию `reply_all: true`; сохраняются исходная беседа и ссылки на письмо.
Можно передать `additional_to`, `additional_cc` и `attachments`. После проверки
используйте `send_draft`. Инструмент `reply_email` по-прежнему отправляет сразу,
но теперь поддерживает HTML и упоминания.

В `send_email`, `create_draft`, `reply_email`, `create_reply_draft` и
`update_draft` упоминания задаются явно:

```json
{
  "id": "<id исходного письма>",
  "reply_all": true,
  "body": "{{mention:owner}}, прошу подготовить оборудование к забору.",
  "mentions": [
    {"key": "owner", "email": "owner@example.com", "display_name": "Ответственный"}
  ],
...
Токены упоминаний размещаются в тексте, а не в HTML-атрибутах, ссылках или скриптах.
Сервер экранирует обычный текст и имена, создаёт HTML-ссылки с уникальными
`OWAAM...` и записывает заголовок `X-Mentions` через расширенное свойство EWS.
Для значка упоминания Outlook нужны **и ссылка, и заголовок**.
Адрес упомянутого добавляется в To, если его ещё нет в To/CC; существующий CC
сохраняется. При упоминаниях тело становится HTML и используется HTML-подпись.
Этот формат проверен на Exchange 2016 и Legacy Outlook для Mac; другие сочетания
клиента и сервера требуют отдельной проверки.

При обновлении черновика пропущенные поля сохраняются; явный `null` очищает
допускающее его поле, а `attachments: []` удаляет вложения. Для упоминаний нужно
передать новое тело. Замена тела без упоминаний очищает старую метку уведомления.
Обёртка MCP теперь сохраняет различие между пропущенным полем и явным пустым
значением — также для обновлений контактов и событий.

`send_draft` сохраняет уникальный `submission_id`, однократно вызывает SendItem
с явной папкой «Отправленные» текущего аккаунта и проверяет появление копии.
`confirmation_timeout_seconds`: по умолчанию 10 секунд, допустимо 0–30;
0 отключает ожидание. Последний сетевой запрос может дополнительно занять время,
ограниченное настройкой тайм-аута Exchange.

- `sent_confirmed`: копия найдена, `id` содержит её новый ID.
- `submitted`: Exchange принял команду, проверка отключена.
- `submitted_unconfirmed`: команда принята, но чтение завершилось ошибкой либо
  копия ещё не видна. Не отправляйте письмо повторно.
- Ошибка `send_outcome_unknown`: отправка завершилась ошибкой либо ответ потерян;
  перед новой попыткой проверьте «Отправленные».

Результат содержит фактический `sent_copy_mailbox`, папку `sent_copy_folder`
и при подтверждении `datetime_sent`. Это подтверждает наличие отправленной
копии, но не доставку адресату или обновление локального кэша Outlook.
Для диагностики используется именованное String-свойство
`OutlookMcpSubmissionId` в наборе `a8b318c0-6eae-4c97-91aa-5ef142043c2b`.
Автоматические тесты не отправляют настоящих писем.
